...
void buildRT() {
	...
	if (sb->rootNode == 0) { // inodes have not been initialized
		printf("-Root inode is uninitialized, nothing to build.\n");
		return;
	}
...
int initFBList(int nBytes) {
	...
	int i;
	for (i = 0; i < bNum; i++) {
		freeblock *newFB = calloc(BLOCKSIZE, 1);
...
void insertFB(int blockNum) {
	freeblock *fp = malloc(BLOCKSIZE);
	readBlock(mountedDisk, sb->nextFB, fp);

	while (fp->nextBlockNum != 0) {
		readBlock(mountedDisk, fp->nextBlockNum, fp);
	}

	fp->nextBlockNum = blockNum;
	writeBlock(mountedDisk, fp->blockNum, fp);
}

int printFB(freeblock *fb) {
	printf("\n] PRINTING FREE BLOCK\n");
	printf("-Block type:  %d\n", fb->blockType);
	printf("-Block num: %d\n", fb->blockNum);
...
int insertDB(inode *inodePtr, FileExtent *new) {
	...
	if (inodePtr->data == 0) {
		...
		readBlock(mountedDisk, inodePtr->data, dbPtr);

		while (dbPtr->next != 0) {
			readBlock(mountedDisk, dbPtr->next, dbPtr);
		}
		...
		if (dbPtr->next == 0) {
			...
			// write new DB
			writeBlock(mountedDisk, dbPtr->next, new);

			// update previous DB
			writeBlock(mountedDisk, dbPtr->blockNum, dbPtr);
			free(dbPtr);
			return 0;
		}
...
void deallocateDB(int newBlock) {
	...
	writeBlock(mountedDisk, newFB->blockNum, newFB);
}
...
int main() {
	...
	memset(writeBuf, 1, BLOCKSIZE);
	res = tfs_writeFile(fd3, writeBuf, BLOCKSIZE);
	printIN(fd3);
